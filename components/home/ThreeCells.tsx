...
import { ArrowRight, Layers, Bot, Terminal, BookOpenCheck, CheckCircle2 } from "lucide-react";
...
export function ThreeCells() {
  ...
  return (
    <section className="py-24 sm:py-32 bg-slate-50/70 border-t border-slate-200/80 relative dark:bg-slate-900/50 dark:border-slate-800 transition-colors duration-300">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-2xl text-center mb-16 sm:mb-20">
          ...
          <p className="mt-4 text-base sm:text-lg leading-relaxed text-slate-600 dark:text-slate-400">
            Specialized student wings dedicated to driving high-impact technical initiatives across the department.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-7xl mx-auto">
          {cellCards.map((cell) => (
            <div 
              key={cell.id} 
              className={`group relative flex flex-col justify-between overflow-hidden rounded-3xl border border-slate-200/90 bg-white p-7 sm:p-8 shadow-xs transition-all duration-300 hover:-translate-y-1.5 hover:shadow-xl hover:shadow-slate-200/60 dark:border-slate-800 dark:bg-slate-900/80 dark:hover:shadow-black/40 ${cell.hoverBorder}`}
            >
              ...
              <div>
                ...
                <div className="flex items-center justify-between gap-4 mb-6">
                  <div className={`flex h-13 w-13 items-center justify-center rounded-2xl border shadow-xs transition-transform duration-300 group-hover:scale-105 ${cell.iconBg}`}>
                    <cell.icon className="h-6 w-6" aria-hidden="true" />
                  </div>
                  <span className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 font-mono text-[11px] font-bold tracking-wider uppercase ${cell.accentPill}`}>
              ...
              {/* Bottom Action Link */}
              <div className="mt-8 pt-6 border-t border-slate-200/70 dark:border-slate-800/80">
                <Link 
                  href={cell.href}
                  className="inline-flex items-center text-sm font-semibold text-blue-600 group-hover:text-blue-700 dark:text-blue-400 dark:group-hover:text-blue-300"
                >
